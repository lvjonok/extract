...
from extract.utils.pytorch_utils import Updater
...
class BaseModel(nn.Module):
    ...
    def freeze(self):
        """Freezes certain model components for BC finetuning. Defaults to doing nothing, will be overriden if needed."""
        return
    ...
    def log_outputs(
        self, model_output, inputs, losses, step, log_images, phase, **logging_kwargs
    ):
    ...
    def load_state_dict(self, state_dict, strict=True):
        # simply load the state dict like normal but account for possible torch.compiled models in the checkpoint which have the _orig_mod. prefix for each weight
    ...
    def _load_weights(self, weight_loading_info):
        ...
        def get_filtered_weight_dict(checkpoint_path, scope):
            if os.path.isfile(checkpoint_path):
                ...
                if not filtered_state_dict:
                    raise ValueError(
                        "No variable with scope '{}' found in checkpoint '{}'!".format(
                ...
                raise ValueError(
                    "Cannot find checkpoint file '{}' for loading '{}'.".format(
        ...
        for loading_op in weight_loading_info:
            print(
                (
                    "=> loading '{}' from checkpoint '{}'".format(
                        loading_op[1], loading_op[2]
                    )
                )
            )
            filtered_weight_dict = get_filtered_weight_dict(
                checkpoint_path=loading_op[2], scope=loading_op[1]
            )
    ...
    def log_gradients(self, step, phase):
        grad_norms = list(
            [torch.norm(p.grad.data) for p in self.parameters() if p.grad is not None]
        )
        if len(grad_norms) == 0:
            return
    ...
    def _compute_total_loss(losses):
        total_loss = torch.stack(
            [
                loss[1].value * loss[1].weight
                for loss in filter(lambda x: x[1].weight > 0, losses.items())
            ]
        ).sum()
