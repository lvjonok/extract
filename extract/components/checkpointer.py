...
import numpy as np
import torch
from extract.utils.general_utils import str2int
...
class CheckpointHandler:
    ...
    def load_weights(
        ...
        if os.path.isfile(weights_file):
            ...
            ### For compatability with torch.compiled model checkpoint
            new_state_dict = {}
            ...
            if load_opt:
                try:
                    ...
                    if not strict:
                        ...
                        raise e
                if grad_scaler is not None and "grad_scaler" in checkpoint:
                    grad_scaler.load_state_dict(checkpoint["grad_scaler"])
                elif grad_scaler is not None and "grad_scaler" not in checkpoint:
                    print(
                        "Could not load grad scaler because it was not saved in the checkpoint"
                    )
            print(
                (
