...
class VideoDataset(Dataset):
    ...
    def __init__(self, *args, resolution, **kwargs):
        ...
        self.randomize_length = (
            self.spec.randomize_length if "randomize_length" in self.spec else False
        )
        self.crop_subseq = (
            "crop_rand_subseq" in self.spec and self.spec.crop_rand_subseq
        )
    ...
    def __getitem__(self, index):
        ...
        if self.crop_subseq:
            end_ind = np.argmax(
                data.pad_mask * np.arange(data.pad_mask.shape[0], dtype=np.float32), 0
            )
        ...
        end_ind = (
            ...
            if self.randomize_length or self.crop_subseq
            else self.spec.max_seq_len - 1
        )
        end_ind, data = self._sample_max_len_video(
            data,
            end_ind,
            target_len=self.spec.subseq_len
            if self.crop_subseq
        ...
        try:
            with h5py.File(path, "r") as F:
                ...
                if key + "/images" in F:
                    ...
                    data.images = np.zeros(
                        (data.states.shape[0], 2, 2, 3), dtype=np.uint8
                    )
        except:
    ...
    def _crop_rand_subseq(self, data, end_ind, length):
        ...
        assert (
            length <= end_ind + 1
        )  # sequence needs to be longer than desired subsequence length
    ...
    def _sample_max_len_video(self, data_dict, end_ind, target_len):
        ...
        data_dict = map_dict(
            lambda tensor: self._maybe_pad(tensor, offset, target_len), data_dict
        )
        ...
        if len > target_length:
            ...
            return np.concatenate(
                (
                    val,
                    np.zeros(
                        [int(target_length - len)] + list(val.shape[1:]),
                        dtype=val.dtype,
                    ),
                )
            )
    ...
    def _randomize_length(self, start_ind, end_ind, data_dict):
        ...
        length = 3 + int(
            np.random.rand() * (end_ind - 2)
        )  # The length of the seq is from 2 to total length
        chop_length = int(
            np.random.rand() * (end_ind + 1 - length)
        )  # from 0 to the reminder
        end_ind = length - 1
        pad_mask = np.logical_and(
            (np.arange(self.spec["max_seq_len"]) <= end_ind),
            (np.arange(self.spec["max_seq_len"]) >= start_ind),
        ).astype(np.float32)

        # Chop off the beginning of the arrays
        def pad(array):
            array = np.concatenate(
                [array[chop_length:], np.repeat(array[-1:], chop_length, 0)], 0
            )
            array[end_ind + 1 :] = 0
            return array
...
class PreloadVideoDataset(VideoDataset):
    ...
    def _load_data(self):
        ...
        return [
            super(PreloadVideoDataset, self)._get_raw_data(i)
...
class MixedVideoDataset(GlobalSplitVideoDataset):
    ...
    def _load_h5_files(self, unused_dir):
        ...
        files = [
            super(MixedVideoDataset, self)._load_h5_files(dir)
            for dir in self.spec.data_dirs
        ]
...
class RandomVideoDataset(GeneratedVideoDataset):
    def get_sample(self):
        data_dict = AttrDict()
        data_dict.images = np.random.rand(
            self.spec["max_seq_len"], 3, self.img_sz, self.img_sz
        ).astype(np.float32)
        data_dict.states = np.random.rand(
            self.spec["max_seq_len"], self.spec["state_dim"]
        ).astype(np.float32)
        data_dict.actions = np.random.rand(
            self.spec["max_seq_len"] - 1, self.spec["n_actions"]
        ).astype(np.float32)
