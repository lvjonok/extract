...
from extract.utils.general_utils import batch_apply, ParamDict
from extract.utils.pytorch_utils import (
    get_constant_parameter,
    ResizeSpatial,
    RemoveSpatial,
)
from extract.models.skill_prior_mdl import SkillPriorMdl, ImageSkillPriorMdl
from extract.modules.subnetworks import Predictor, BaseProcessingLSTM, Encoder
...
class ClSPiRLMdl(SkillPriorMdl):
    ...
    def build_network(self):
        ...
        decoder_input_size = self.enc_size + self._hp.nz_vae
        ...
        self.log_sigma = get_constant_parameter(0.0, learnable=False)
    ...
    def decode(self, z, cond_inputs, steps, inputs=None):
        ...
        decode_inputs = torch.cat(
            (seq_enc[:, :steps], z[:, None].repeat(1, steps, 1)), dim=-1
        )
        if self._hp.use_language:
            decode_inputs = torch.cat(
                (decode_inputs, inputs.lang[:, 0:1].repeat(1, steps, 1)), dim=-1
            )
    ...
    def _run_inference(self, inputs):
        ...
        inf_input = torch.cat(
            (self._regression_targets(inputs), self._get_seq_enc(inputs)), dim=-1
        )
        if self._hp.use_language:
