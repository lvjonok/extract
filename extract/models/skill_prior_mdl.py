...
from extract.components.checkpointer import load_by_key, freeze_modules
from extract.modules.losses import KLDivLoss, NLL
...
from extract.utils.vis_utils import fig2img
...
from extract.modules.mdn import MDN, GMM
from extract.modules.flow_models import ConditionedFlowModel
...
class SkillPriorMdl(BaseModel, ProbabilisticModel):
    ...
    def _default_hparams(self):
        ...
        default_dict = ParamDict(
            {
                ...
                "encoder_max_range": float("inf"),  # max range of encoder mean output
            }
        )
        ...
        default_dict.update(
            {
                "reconstruction_mse_weight": 1.0,  # weight of MSE reconstruction loss
                "kl_div_weight": 1.0,  # weight of KL divergence loss
        ...
        # what parts to actually train, for imitation finetuning
        default_dict.update(
            {
                "freeze_decoder": False,  # if True, freezes decoder
                "freeze_encoder": False,  # if True, freezes encoder (inference net)
            }
        )
    ...
    def build_network(self):
        ...
        decoder_input_size = self._hp.action_dim + self._hp.nz_vae
    ...
    def forward(self, inputs, use_learned_prior=False):
        ...
        output.q_hat = self.compute_learned_prior(
            self._learned_prior_input(inputs), first_only=True
        )
        ...
        output.reconstruction = self.decode(
            output.z,
    ...
    def decode(self, z, cond_inputs, steps, inputs=None):
        ...
        if self._hp.use_language:
            lstm_static_inputs.update(lang=inputs.lang[:, 0])
        return self.decoder(
            lstm_initial_inputs=AttrDict(x_t=lstm_init_input),
            steps=steps,
            lstm_static_inputs=lstm_static_inputs,
    ...
    def run(self, inputs, use_learned_prior=True):
        ...
        if not self._action_plan:
            ...
            z = (
                self.compute_learned_prior(
                    self._learned_prior_input(inputs), first_only=True
                ).sample()
                if use_learned_prior
                else Gaussian(
                    torch.zeros((1, self._hp.nz_vae * 2), device=self.device)
                ).sample()
            )
            ...
            actions = self.decode(
                z, cond_inputs=input_obs, steps=self._hp.n_rollout_steps
            )[0]
    ...
    def load_weights_and_freeze(self):
        ...
        if self._hp.embedding_checkpoint is not None:
            ...
            self.load_state_dict(
                load_by_key(
                    self._hp.embedding_checkpoint, "q", self.state_dict(), self.device
                )
            )
            freeze_modules(
                [
                    self.decoder,
                    self.decoder_input_initalizer,
                    self.decoder_hidden_initalizer,
                    self.q,
                ]
            )

    def freeze(self):
        if self._hp.freeze_decoder:
    ...
    def _build_decoder_initializer(self, size):
        if self._hp.cond_decode:
            ...
            # TODO: closed loop model might not work with language because self.prior_input_size takes it into account
            # and yet we count it as a static input
            return Predictor(
                self._hp,
                input_size=self.prior_input_size,
                output_size=size,
            ...
            class FixedTrainableInitializer(nn.Module):
                def __init__(self, hp):
                    ...
                    self.val = nn.Parameter(
                        torch.zeros(
                            (1, size), requires_grad=True, device=self._hp.device
                        )
                    )
    ...
    def _build_prior_ensemble(self):
        return nn.ModuleList(
            [self._build_prior_net() for _ in range(self._hp.n_prior_nets)]
        )
    ...
    def _run_inference(self, inputs):
        inf_input = self._regression_targets(inputs)
        if self._hp.use_language:
            inf_input = torch.cat((inf_input, inputs.lang), dim=-1)
        if self._hp.cond_decode:
            # TODO: another area to possibly fix with language and cond_decode
            inf_input = torch.cat(
                (
                    inf_input,
                    self._learned_prior_input(inputs)[:, None].repeat(
                        1, inf_input.shape[1], 1
                    ),
                ),
                dim=-1,
            )
        return MultivariateGaussian(
            self.q(inf_input)[:, -1], max_mu=self._hp.encoder_max_range
        )
    ...
    def compute_learned_prior(self, inputs, first_only=False):
        ...
        prior_results = [
            self._compute_learned_prior(prior, input_batch)
    ...
    def _compute_learned_prior(self, prior_mdl, inputs):
        if self._hp.learned_prior_type == "gmm":
            ...
            return MultivariateGaussian(
                prior_mdl(inputs), max_mu=self._hp.encoder_max_range
            )
    ...
    def _compute_learned_prior_loss(self, model_output):
        ...
        loss.breakdown = torch.stack(
            [
                chunk.mean()
                for chunk in torch.chunk(loss.breakdown, self._hp.n_prior_nets)
            ]
        )
        return loss
    ...
    def _update_beta(self, kl_div):
        ...
        beta_loss = (
            self._log_beta().exp() * (self._hp.target_kl - kl_div).detach().mean()
        )
    ...
    def _learned_prior_input(self, inputs):
        if self._hp.use_language:
            return torch.cat((inputs.states[:, 0], inputs.lang), dim=-1)
    ...
    def evaluate_prior_divergence(self, state):
        ...
        assert (
            self._hp.n_prior_nets > 1
        )  # need more than one prior in ensemble to evaluate divergence
        if not isinstance(state, torch.Tensor):
            state = torch.tensor(state, device=self._hp.device)
        if len(state.shape) == 1:
            state = state[None]
        state_batch = (
            state.repeat(self._hp.n_prior_nets, 1)
            if len(state.shape) == 1
            else state.repeat(self._hp.n_prior_nets, 1, 1, 1)
        )
...
class ImageSkillPriorMdl(SkillPriorMdl):
    ...
    def _build_prior_net(self):
        if self._hp.use_language:
            return LangConditionedPrior(
                self._hp, self._updated_encoder_params(), super()._build_prior_net()
            )
    ...
    def _build_inference_net(self):
        ...
        in_dim = self._hp.action_dim + self._hp.nz_enc
        if self._hp.use_language:
    ...
    def _run_inference(self, inputs):
        ...
        inf_input = torch.cat(
            (inputs.actions, enc_cond[:, None].repeat(1, inputs.actions.shape[1], 1)),
            dim=-1,
        )
        if self._hp.use_language:
            inf_input = torch.cat((inf_input, inputs.lang), dim=-1)
    ...
    def unflatten_obs(self, raw_obs):
        ...
        return AttrDict(
            ...
            prior_obs=raw_obs[:, self.state_dim :].reshape(
                raw_obs.shape[0],
                3 * self._hp.n_input_frames,
                self._hp.prior_input_res,
                self._hp.prior_input_res,
            ),
        )
    ...
    def prior_input_size(self):
        if self._hp.use_language:
            return self._hp.lang_dim + self._hp.nz_mid
...
class LangConditionedPrior(nn.Module):
    """
    Combines the image and language
    """

    def __init__(self, hp, encoder_params, flattened_prior_net):
        super().__init__()
        self._image_processor = nn.Sequential(
            ResizeSpatial(hp.prior_input_res),
            Encoder(encoder_params),
            RemoveSpatial(),
        )

        self._prior_net = flattened_prior_net
        self._img_stats = AttrDict(
            mean=MaxSizeQueue(1000),
            min=MaxSizeQueue(1000),
            max=MaxSizeQueue(1000),
            std=MaxSizeQueue(1000),
        )
        self._lang_stats = AttrDict(
            mean=MaxSizeQueue(1000),
            min=MaxSizeQueue(1000),
            max=MaxSizeQueue(1000),
            std=MaxSizeQueue(1000),
    ...
    def forward(self, inputs):
        ...
        # save the mean, min, max, std of image_out and lang
        self._img_stats.mean.add(image_out.mean().item())
        self._img_stats.min.add(image_out.min().item())
        self._img_stats.max.add(image_out.max().item())
        self._img_stats.std.add(image_out.std().item())
        self._lang_stats.mean.add(lang.mean().item())
        self._lang_stats.min.add(lang.min().item())
        self._lang_stats.max.add(lang.max().item())
        self._lang_stats.std.add(lang.std().item())

        flattened_input = torch.cat((image_out, lang), dim=-1)
        return self._prior_net(flattened_input)

    def get_logging_stats(self):
        return AttrDict(
            img_stats=self._img_stats,
            lang_stats=self._lang_stats,
        )
...
class SkillSpaceLogger(Logger):
    ...
    def _draw_learned_prior(self, ax, prior, color):
        if isinstance(prior, GMM):
            [
                self._draw_gaussian(ax, component.tensor(), color, ten2ar(weight))
