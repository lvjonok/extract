...
from extract.utils.general_utils import ConcatSequential, HasParameters
...
class Block(nn.Sequential, HasParameters):
    ...
    def complete_block(self):
        if self.params.normalization is not None:
            ...
            elif self.params.normalization == "layer":
                normalization = nn.LayerNorm
                self.params.normalization_params = {}
                if hasattr(self, "conv"): # don't norm the conv layers
                    normalization = None
            else:
                raise ValueError(
