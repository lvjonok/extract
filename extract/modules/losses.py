...
from extract.utils.general_utils import AttrDict, get_dim_inds
...
class DiscreteKLLoss(Loss):
    def compute(self, estimates, targets):
        assert (
            estimates.shape == targets.shape
        ), "Input {} and targets {} for Discrete KL loss need to have identical shape!".format(
            estimates.shape, targets.shape
        )
        if not isinstance(targets, torch.Tensor):
            targets = torch.tensor(
        ...
        kl_loss = torch.nn.KLDivLoss(reduction="none")(estimates, targets)
        return kl_loss
