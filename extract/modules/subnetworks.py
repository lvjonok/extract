...
from extract.modules.layers import (
    BaseProcessingNet,
    ConvBlockEnc,
    ConvBlockDec,
    init_weights_xavier,
...
from extract.modules.recurrent_modules import BaseProcessingLSTM, BidirectionalLSTM
from extract.modules.variational_inference import Gaussian
from extract.modules.variational_inference import SequentialGaussian_SharedPQ
from extract.modules.variational_inference import UnitGaussian
...
from extract.utils.general_utils import broadcast_final, AttrDict
...
class ConvEncoder(nn.Module):
    def __init__(self, hp):
        ...
        # add output layer
        print("doing hacky thing in ConvEncoder in subnetworks.py for image size if it's not a power of 2")
        self.net.add_module("head", nn.Conv2d(hp.ngf * 2 ** (n - 3), hp.nz_enc, 4 if math.log2(hp.img_sz).is_integer() else 2))
    ...
    def _updated_encoder_params(self):
        ...
        return params.overwrite(
            AttrDict(
                ...
                ngf=self._hp.ngf,  # number of feature maps in first layer
                nz_enc=self._hp.nz_enc,  # number of dimensions in encoder-latent space
