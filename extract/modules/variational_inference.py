...
from extract.utils.general_utils import batch_apply
...
class Categorical:
    def __init__(self, logits=None, probs=None):
        self.dist = FixedCategorical(logits=logits, probs=probs)
    ...
    def kl_divergence(self, other):
        return self.dist.probs * (self.dist.logits - other.dist.logits)

    def detach(self):
        return type(self)(logits=self.dist.logits.detach())
...
class Gaussian:
    ...
    def __init__(self, mu, log_sigma=None, max_mu=float("inf")):
        """
        ...
        self.mu = (
            torch.clamp(mu, min=-max_mu, max=max_mu)
            if isinstance(mu, torch.Tensor)
            else np.clip(mu, a_min=-max_mu, a_max=max_mu)
        )
        self.log_sigma = (
