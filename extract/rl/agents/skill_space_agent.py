...
from extract.utils.general_utils import ParamDict, split_along_axis, AttrDict
from extract.utils.pytorch_utils import map2torch, map2np, no_batchnorm_update
from extract.models.skill_prior_mdl import LangConditionedPrior
...
class SkillSpaceAgent(BaseAgent):
    ...
    def __init__(self, config):
        ...
        self.load_model_weights(
            self._policy, self._hp.model_checkpoint, self._hp.model_epoch
        )
    ...
    def _act(self, obs, extra_info):
        ...
        if not self.action_plan:
            ...
            with no_batchnorm_update(self._policy) if obs.shape[
                0
            ] == 1 else contextlib.suppress():
                actions = self._policy.decode(
                    ...
                    map2torch(extra_info, self._hp.device)
                    if extra_info
                    else extra_info,
                )
    ...
    def _split_obs(self, obs):
        ...
        return AttrDict(
            cond_input=obs[
                :, : -self._policy.latent_dim
            ],  # condition decoding on state
    ...
    def _update_model_params(self):
        self._hp.model_params.device = (
            self._hp.device
        )  # transfer device to low-level model
        self._hp.model_params.batch_size = 1  # run only single-element batches

    def _act_rand(self, obs, extra_info):
        return self._act(obs, extra_info)

    def log_outputs(self, logging_stats, rollout_storage, logger, log_images, step):
        # check if the prior is the LangConditioned prior and log the stats
        if isinstance(self._policy.p[0], LangConditionedPrior):
            stats = self._policy.p[0].get_logging_stats()
            for key in stats:
                for inner_key in stats[key]:
                    logging_stats[f"prior/{key}/{inner_key}"] = np.mean(
                        stats[key][inner_key]
                    )
