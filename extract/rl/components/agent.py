...
from extract.utils.pytorch_utils import RAdam, remove_grads, map2np, map2torch
from extract.utils.vis_utils import add_caption_to_img, add_captions_to_seq
from extract.rl.components.normalization import DummyNormalizer
from extract.rl.components.policy import Policy
from extract.components.checkpointer import CheckpointHandler
from extract.rl.utils.mpi import sync_grads
...
class BaseAgent(nn.Module):
    ...
    def act(self, obs, extra_info=None):
        ...
        if self._rand_act_mode:
            agent_output = self._act_rand(obs, extra_info=extra_info)
        else:
            agent_output = self._act(obs, extra_info=extra_info)
        extra_env_info = self.add_extra_env_info()
        agent_output.extra_env_info = extra_env_info
        return agent_output

    def add_extra_env_info(self):
        """Adds extra info after acting"""
        return AttrDict()
...
class HierarchicalAgent(BaseAgent):
    ...
    def act(self, obs, extra_info=None):
        ...
        if self._perform_hl_step_now:
            ...
            self._last_hl_output = self.hl_agent.act(obs_input, extra_info)
    ...
    def _act_rand(self, obs, extra_info=None):
        ...
        with self.hl_agent.rand_act_mode():
            return self.act(obs, extra_info=extra_info)
