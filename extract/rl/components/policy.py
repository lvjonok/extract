...
from extract.utils.general_utils import ParamDict, AttrDict, nan_hook
...
class Policy(nn.Module):
        ...
        default_dict = ParamDict(
            {
                ...
                "max_action_range": 1.0,  # for cont. actions this defines a symmetric action range [-x, x]
                "squash_output_dist": False,  # do not tanh adjust log prob if set to False
            }
        )
        return default_dict

    def forward(self, obs, extra_info):
        output_dist = self._compute_action_dist(obs, extra_info)
        action = output_dist.rsample()
        #if self._is_train:  # only sample on rollout
        #    action = output_dist.rsample()
        #else:
        #    action = output_dist.mu  # otherwise just use the mean
    ...
    def sample_rand(self, unused_obs, extra_info):
        """Samples random action."""
        with torch.no_grad():
            # TODO: implement proper ActionSpace class with sample() method
            return AttrDict(
                action=self._hp.max_action_range
                * (2 * torch.rand((self._hp.action_dim,)) - 1.0),
                log_prob=-torch.log(torch.tensor(2 * self._hp.max_action_range))
                * self._hp.action_dim,
