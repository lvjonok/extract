...
from extract.utils.general_utils import AttrDict, RecursiveAverageMeter, ParamDict
...
class SplitObsReplayBuffer(ReplayBuffer):
    ...
    def preprocess_experience(self, experience_batch):
        ...
        return filtered_experience_batch

    def append(self, experience_batch):
        filtered_experience_batch = self.preprocess_experience(experience_batch)
...
class IntImageUniformReplayBuffer(UniformReplayBuffer):
    def append(self, experience_batch):
        # unnormalize the observation to be in [0, 255] and then turn it into an int
        experience_batch.observation = [
            ((o + 1) / 2 * 255).astype(np.uint8) for o in experience_batch.observation
        ]
        experience_batch.observation_next = [
            ((o + 1) / 2 * 255).astype(np.uint8)
            for o in experience_batch.observation_next
        ]
        return super().append(experience_batch)

    def sample(self, n_samples, filter=None):
        samples = super().sample(n_samples, filter)
        samples.observation = samples.observation.astype(float) / 255 * 2 - 1
        samples.observation_next = samples.observation_next.astype(float) / 255 * 2 - 1
        return samples


class LanguageIntImageUniformReplayBuffer(IntImageUniformReplayBuffer):
    def append(self, experience_batch):
        experience_batch.lang = [np.array(o) for o in experience_batch.lang]
        return super().append(experience_batch)


class IntImageSplitObsUniformReplayBuffer(SplitObsUniformReplayBuffer):
    # for low level policy if we ever need to train
    def append(self, experience_batch):
        filtered_experience_batch = self.preprocess_experience(experience_batch)
        assert (filtered_experience_batch.observation[0].max() <= 1).all() and (
            filtered_experience_batch.observation[0].min() >= -1
        ).all()
        # unnormalize the observation to be in [0, 255] and then turn it into an int
        filtered_experience_batch.observation = [
            ((o + 1) / 2 * 255).astype(np.uint8)
...
class RolloutStorage:
    ...
    def rollout_stats(self):
        ...
        for rollout in self.rollouts:
            ...
            # TODO: debug this line here, not recording episode success
            if "success" in rollout.info[-1]:
                stats.update(AttrDict(success=int(rollout.info[-1]["success"])))
        return stats.avg
