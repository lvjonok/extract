...
from extract.utils.general_utils import listdict2dictlist, AttrDict, ParamDict, obj2np
from extract.modules.variational_inference import MultivariateGaussian
from extract.rl.utils.reward_fcns import sparse_threshold
...
class Sampler:
    ...
    def __init__(self, config, env, agent, logger, max_episode_len):
        ...
        self._obs = None
        self._lang = None
        self._episode_step, self._episode_reward, self._episode_success = 0, 0, 0

    def _default_hparams(self):
        return ParamDict({})

    def init(self, is_train, env_id=None):
        """Starts a new rollout. Render indicates whether output should contain image."""
        with self._env.val_mode(env_id) if not is_train else contextlib.suppress():
            with self._agent.val_mode() if not is_train else contextlib.suppress():
                self._episode_reset()

    def sample_action(self, obs, extra_info=None):
        return self._agent.act(obs, extra_info)

    def sample_batch(self, batch_size, is_train=True, global_step=None, store_ll=None):
            ...
            with self._agent.val_mode() if not is_train else contextlib.suppress():
                with self._agent.rollout_mode():
                    while step < batch_size:
                        ...
                        # optionally add language
                        if hasattr(self._env, "lang"):
                            experience_batch[-1].update(
                                AttrDict(lang=self._env.lang.squeeze(1))
                            )
                        ...
                        self._episode_success += info.get("success", 0)
                        if hasattr(self._env, "lang"):
                            self._lang = self._env.lang
    ...
    def sample_episode(self, is_train, render=False, env_id=None):
        """Samples one episode from the environment."""
        self.init(is_train, env_id=env_id)
        episode, done = [], False
        with self._env.val_mode(env_id) if not is_train else contextlib.suppress():
            with self._agent.val_mode() if not is_train else contextlib.suppress():
                with self._agent.rollout_mode():
                    while not done and self._episode_step < self._max_episode_len:
                        ...
                        obs, reward, done, info = self._env.step(agent_output.action)
                        info.update(agent_output.extra_env_info)
                        obs = self._postprocess_obs(obs)
                        episode.append(
                            AttrDict(
                                observation=self._obs,
                        ...
                        self._episode_step += 1
        episode[-1].done = (
            True  # make sure episode is marked as done at final time step
        )
    ...
    def _episode_reset(self, global_step=None):
        ...
        self._episode_step, self._episode_reward, self._episode_success = 0, 0.0, 0
...
class HierarchicalSampler(Sampler):
    ...
    def sample_batch(self, batch_size, is_train=True, global_step=None, store_ll=True):
        ...
        with self._env.val_mode() if not is_train else contextlib.suppress():
            with self._agent.val_mode() if not is_train else contextlib.suppress():
                with self._agent.rollout_mode():
                    while hl_step < batch_size or (
                        len(ll_experience_batch) <= 1 and store_ll
                    ):
                        # perform one rollout step
                        extra_info = AttrDict(lang=self._lang)
                        agent_output = self.sample_action(
                            self._obs, extra_info=extra_info
                        )
                        agent_output = self._postprocess_agent_output(agent_output)
                        obs, reward, done, info = self._env.step(agent_output.action)
                        obs = self._postprocess_obs(obs)
        ...
        return (
            AttrDict(
                ...
                ll_batch=(
                    listdict2dictlist(ll_experience_batch[:-1]) if store_ll else {}
...
class ImageAugmentedSampler(Sampler):
    ...
    def _postprocess_obs(self, obs):
        img = self._env.render(mode="rgb_array").transpose(2, 0, 1) * 2.0 - 1.0
