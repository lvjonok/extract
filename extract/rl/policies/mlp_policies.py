...
from extract.modules.subnetworks import Predictor, HybridConvMLPEncoder, Encoder
...
from extract.utils.pytorch_utils import RemoveSpatial
...
class MDNPolicy(MLPPolicy):
    ...
    def _build_network(self):
        assert (
            self._hp.num_gaussians is not None
        )  # need to specify number of mixture components for policy
...
class SplitObsMLPPolicy(MLPPolicy):
    ...
    def _compute_action_dist(self, raw_obs, extra_info):
        if self._hp.discard_part == "front":
            return super()._compute_action_dist(
                raw_obs[:, self._hp.unused_obs_size :], extra_info
            )
...
class ConvPolicy(MLPPolicy):
    ...
    def _compute_action_dist(self, obs, extra_info):
        return super()._compute_action_dist(
            obs[:, self._hp.input_dim :].reshape(
...
class LangConditionedConvPolicy(ConvPolicy):
    ...
    def _build_network(self):
        return HybridConvMLPEncoder(
            copy.deepcopy(self._hp).overwrite(
                AttrDict(input_dim=self._hp.lang_dim, output_dim=self.mlp_output_size)
            )
        )
    ...
    def _compute_action_dist(self, obs, extra_info):
        ...
        return MLPPolicy._compute_action_dist(self, split_obs, extra_info)
...
class HybridConvMLPPolicy(ConvPolicy):
    ...
    def _build_network(self):
        return HybridConvMLPEncoder(
            self._hp.overwrite(AttrDict(output_dim=self.mlp_output_size))
        )
    ...
    def _compute_action_dist(self, obs, extra_info):
        ...
        return super()._compute_action_dist(split_obs, extra_info)
