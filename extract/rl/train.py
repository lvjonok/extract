...
from extract.rl.components.params import get_args
from extract.train import (
    set_seeds,
    make_path,
    datetime_str,
    save_config,
...
from extract.rl.utils.mpi import (
    update_with_mpi_config,
    set_shutdown_hooks,
    mpi_sum,
    mpi_gather_experience,
)
from extract.rl.utils.wandb import WandBLogger
from extract.rl.utils.rollout_utils import RolloutSaver
...
class RLTrainer:
    ...
    def __init__(self, args):
        ...
        if self.is_chef:
            ...
            self.logger = self.setup_logging(self.conf, self.log_dir, args.run_group)
        else:
        ...
        self.conf.env.gpu = self.args.gpu
        ...
        # make the agent have more updates per iteration if extra number of workers
        if self.use_multiple_workers:
            self.agent._hp.update_iterations *= self.conf.mpi.num_workers
        ...
        self.agent = torch.compile(self.agent)  # compile the agent
        ...
        elif args.mode == "prior_rollout":
            self.generate_prior_rollouts()
        else:
                ...
                save_checkpoint(
                    {
                        ...
                        "state_dict": getattr(
                            self.agent, "_orig_mod", self.agent
                        ).state_dict(),  # _orig_mod is for torch.compiled models
                    },
                    os.path.join(self._hp.exp_path, "weights"),
                    "weights_ep0.pth",  # always epoch 0 so we overwrite
    ...
    def generate_prior_rollouts(self):
    ...
    def warmup(self):
        ...
        with self.agent.rand_act_mode():
            self.sampler.init(is_train=True)
            warmup_experience_batch, warmup_env_steps = self.sampler.sample_batch(
                batch_size=int(self._hp.n_warmup_steps / self.conf.mpi.num_workers),
                store_ll=False,  # DO NOT STORE LL stuff, not needed
            )
            if self.use_multiple_workers:
                warmup_experience_batch = mpi_gather_experience(warmup_experience_batch)
                self.global_step += mpi_sum(warmup_env_steps)
        if self.is_chef:
    ...
    def setup_logging(self, conf, log_dir, run_group):
        if not self.args.dont_save:
            ...
            logger = None
            if (
                self.args.mode == "train" or self.args.mode == "prior_rollout"
            ):  # prior rollout to save videos
