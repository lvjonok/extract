...
from extract.utils import ndim
...
class MiniTrainer:
    def __init__(self, model=None, step_fn=None, parameters=None):
        ...
        self.optimizer = optim.Adam(
            filter(lambda p: p.requires_grad, parameters), lr=0.001
        )
...
def dict_concat(d1, d2):
    if not set(d1.keys()) == set(d2.keys()):
        raise ValueError(
            "Dict keys are not equal. got {} vs {}.".format(d1.keys(), d2.keys())
        )
    for key in d1:
...
def batch_apply(
    ...
    reshape_to = make_recursive(
        lambda tensor: tensor.view([batch * time] + list(tensor.shape[2:]))
    )
    reshape_from = make_recursive(
        lambda tensor: tensor.view([batch, time] + list(tensor.shape[1:]))
    )
...
def find_tensor(tensors, min_dim=None):
    ...
    if isinstance(tensors, list) or isinstance(tensors, tuple):
        ...
        if min_dim is not None:
            success = success and len(tensors.shape) >= min_dim
...
def nan_hook(tensor_struct):
    ...
    def check_nan(t):
        if (isinstance(t, torch.Tensor) and torch.isnan(t).any()) or (
            isinstance(t, np.ndarray) and np.isnan(t).any()
        ):
            import pdb
...
class SkipInputSequential(nn.Sequential):
    ...
    def forward(self, input, skips):
        ...
        skips = [None] + skips[
            ::-1
        ]  # start applying skips after the first decoding layer
        for i, module in enumerate(self._modules.values()):
...
def concat_inputs(*inp):
    ...
    inp = torch.cat(
        [add_n_dims(tensor, max_n_dims - len(tensor.shape)) for tensor in inp], dim=1
    )
    return inp
...
def get_end_ind(pad_mask):
    ...
    end_ind = torch.argmax(
        pad_mask * torch.arange(max_seq_len, dtype=torch.float, device=pad_mask.device),
        1,
    )
...
class ConstantSchedule(Schedule):
    ...
    def _default_hparams(self):
        return super()._default_hparams().overwrite(AttrDict(p=None))
